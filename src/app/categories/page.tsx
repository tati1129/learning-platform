  ...
  return (
    <div className="flex flex-wrap gap-3 min-h-60  p-2 m-3 justify-center ">
      {categories.map((categ) => (
        <div key={categ.id} className="w-1/7 flex m-2  p-3 flex-col items-center rounded-2xl shadow bg-gray-100 center transition-all transform hover:scale-130 hover:bg-pink-100 dark:hover:bg-gray-700
              cursor-pointer duration-300 ease-in-out">
          <img className="w-30 object-cover rounded-md" src={categ.image} alt={categ.name} />
          <p>{categ.name}</p>
        </div>
      ))}
    </div>
